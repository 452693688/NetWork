package com.ui.utiles.time;


import com.ui.utiles.other.NumberUtile;
...
public class CalendarUtile {
     ...
     * 获取前day天
     */
    public static Date getFrontDay(Date date, int day) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        int y = calendar.get(Calendar.YEAR);
        int m = calendar.get(Calendar.MONTH);
        int d = calendar.get(Calendar.DATE);
        return getFrontDay(y, m, d, day);
    }

    public static Date getFrontDay(int year, int month, int day, int differ) {
        Calendar lastDate = Calendar.getInstance();
        lastDate.set(Calendar.YEAR, year);
        lastDate.set(Calendar.MONTH, month);
        lastDate.set(Calendar.DATE, day);
        lastDate.add(Calendar.DATE, differ);
    ...
    //yyyy-MM-dd
    public static String getIntervalMonth(String time) {
        String[] times = time.split("-");
        return getIntervalMonth(
                NumberUtile.getStringToInt(times[0]),
                NumberUtile.getStringToInt(times[1]),
                NumberUtile.getStringToInt(times[2])
        );
    }

    //获取间隔一个月的时间
    public static String getIntervalMonth(int y, int m, int d) {
        Calendar calendar = Calendar.getInstance();
        m -= 1;
        ...
        return getTime(y1, m1, d1);
    }

    public static String getTime(int y, int m, int d) {
        String startM = m + "";
        String startD = d + "";
        if (m < 10) {
            startM = "0" + m;
